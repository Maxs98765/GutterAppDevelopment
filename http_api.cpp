...
esp_err_t handleRotate(httpd_req_t *req)
{
    ...
    std::string cmd = "R";
    if (haveHold) cmd += ":" + std::to_string(hold);

    std::string reply = uno::ask(cmd);
    if (reply.empty()) {
        return sendJson(req, "504 Gateway Timeout",
                        "{\"ok\":false,\"error\":\"the arduino did not answer\"}");
    }

    std::string body = "{\"ok\":true,\"reply\":\"" + jsonEscape(reply) + "\"";
...
esp_err_t handleHold(httpd_req_t *req)
{
    ...
    std::string reply = uno::ask("H:" + std::to_string(ms));
    return sendJson(req, "200 OK",
                    "{\"ok\":true,\"hold_ms\":" + std::to_string(ms) +
                    ",\"reply\":\"" + jsonEscape(reply) + "\"}");
}

esp_err_t handleStatus(httpd_req_t *req)
{
    uno::Status s = uno::status();

    std::string body = "{";
    body += "\"ok\":" + std::string(s.valid ? "true" : "false");
    body += ",\"arduino\":" + std::string(s.valid ? "true" : "false");
    body += ",\"state\":\"" + jsonEscape(s.state) + "\"";
    body += ",\"angle\":" + std::to_string(s.angle);
    body += ",\"remaining_ms\":" + std::to_string(s.remainingMs);
    body += ",\"camera\":" + std::string(uvc::connected() ? "true" : "false");
    body += ",\"frames\":" + std::to_string(uvc::framesSeen());
    if (!s.valid && !s.raw.empty()) {
        body += ",\"raw\":\"" + jsonEscape(s.raw) + "\"";
    }
    body += "}";
